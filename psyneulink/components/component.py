...
from psyneulink.globals.keywords import AUTO_ASSIGN_MATRIX, COMMAND_LINE, COMPONENT_INIT, CONTEXT, CONTROL_PROJECTION, DEFERRED_INITIALIZATION, FUNCTION, FUNCTION_CHECK_ARGS, FUNCTION_PARAMS, INITIALIZING, INIT_FULL_EXECUTE_METHOD, INPUT_STATES, LEARNING, LEARNING_PROJECTION, LOG_ENTRIES, MODULATORY_SPEC_KEYWORDS, NAME, OUTPUT_STATES, PARAMS, PARAMS_CURRENT, PREFS_ARG, SEPARATOR_BAR, SET_ATTRIBUTE, SIZE, USER_PARAMS, VALUE, VARIABLE, kwComponentCategory
...
# IMPLEMENTATION NOTE:  *** CHECK THAT THIS DOES NOT CAUSE ANY CHANGES AT SUBORDNIATE LEVELS TO PROPOGATE EVERYWHERE
    ...
    def _validate_params(self, request_set, target_set=None, context=None):
        ...
        for param_name, param_value in request_set.items():
            ...
            if not param_name in self.paramClassDefaults:
                ...
                if param_name in {VARIABLE, NAME, VALUE, PARAMS, SIZE, LOG_ENTRIES, FUNCTION_PARAMS}:  # added SIZE here (7/5/17, CW)
                    continue
    ...
    def _instantiate_function(self, function, context=None):
        ...
        if isinstance(function, types.FunctionType) or isinstance(function, types.MethodType):
            self.function_object = UserDefinedFunction(default_variable=function_variable, function=function, context=context)
        elif isinstance(function, Function):
            if not function._variable_not_specified and not iscompatible(function.instance_defaults.variable, function_variable):
                raise ComponentError('Functions not compatible')
